...
function M.detectors.lsp(buf)
  ...
  --- Get the list of workspace folders (and/or LSP root dir) from the LSP client
  --- then return only the one that is parent of the current buffer
...
function M.detectors.pattern(buf, patterns)
  ...
  local pattern = vim.fs.find(function(name)
    for _, p in ipairs(patterns) do
      ...
      --- If pattern start with * (i.e. *test) then return true if
      --- there is any files with the remaining part of the pattern (i.e my_test).
...
--- Get the OS real path of a buffer
---
--- @param buf number
...
--- Get detectors based on LazyRootSpec
...
--- Resolve list of roots for every spec in vim.g.root_spec (or opts.spec if defined).
--- If all is false, immediatly return the moment a spec is able to resolve roots.
